...
import { Lightbulb, Mail, Lock, AlertCircle, ArrowLeft } from 'lucide-react';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const res = await api.auth.googleOAuth({
        ...
        name: email ? email.split('@')[0] : 'OAuth Founder',
      });
      localStorage.setItem('token', res.token);
      localStorage.setItem('user', JSON.stringify(res.user));
      router.push('/dashboard');
      ...
      <div className="w-full max-w-md glass-panel p-8 rounded-2xl border border-white/10 shadow-2xl relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-400 mt-2">
            {forgotMode ? 'Enter email to receive reset credentials' : 'Enter your credentials to access your dashboard'}
          </p>
        </div>
        ...
        {/* Forgot password mode toggle back */}
        {forgotMode && (
          <button
            onClick={() => { setForgotMode(false); setError(''); setInfoMessage(''); }}
            className="w-full mt-4 text-center text-xs text-violet-400 hover:underline cursor-pointer"
          >
